...
import { stripPromptInjections } from "./sanitize.js";
...
export async function analyzeCompany(url, options = {}) {
  ...
  try {
    ...
    const sanitizedContext = stripPromptInjections(context);
    if (!sanitizedContext) {
    ...
    if (tokenConfig.enabled) {
      consumeTokens({
        key: userKey,
        tokens: estimateTokensFromText(sanitizedContext),
        maxTokens: tokenConfig.maxTokens,
        windowMs: tokenConfig.windowMs
      });
    }

    const analysis = await simplifyWithLLM(sanitizedContext, { trace });
